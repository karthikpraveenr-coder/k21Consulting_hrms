...
export default function EditEvent() {
    ...
    useEffect(() => {
        if (eventImage) {
            document.querySelector("#img").setAttribute("src", `https://k21consulting.com/api/storage/app/${eventImage}`);
        }
    }, [eventImage]);
    ...
    const handleSave = async (e) => {
        ...
        try {
            const response = await fetch('https://k21consulting.com/api/public/api/update_event', {
                method: 'POST',
                body: formData,
                headers: {
                    'Authorization': `Bearer ${usertoken}`
    ...
    useEffect(() => {
        axios.get(`https://k21consulting.com/api/public/api/view_editevent/${id}`, {
            headers: {
                'Authorization': `Bearer ${usertoken}`
            }
        })
