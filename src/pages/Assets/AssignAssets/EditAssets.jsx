...
const EditAssets = () => {
    ...
    useEffect(() => {
        const fetchAssetId = async () => {
            try {
                const response = await axios.get(`https://k21consulting.com/api/public/api/edit_assign_assetlist/${id}`, {
                    headers: {
    ...
    const handleSave = async (e) => {
        ...
        try {
            const response = await fetch('https://k21consulting.com/api/public/api/update_assign_asset', {
                method: 'PUT',
    ...
    useEffect(() => {
        const fetchAssetTypes = async () => {
            try {
                const response = await axios.get('https://k21consulting.com/api/public/api/asset_name', {
                    headers: {
                        'Authorization': `Bearer ${usertoken}`
                    }
                });
    ...
    const formattedSelectedAssetType = Array.isArray(selectedAssetType)
        ? selectedAssetType.map(type => ({
            label: formattedAssetTypeDropdown.find(option => String(option.value) === type)?.label,
            value: type
        }))
        : [];
    // ---------------------------------------------------------------------------------------------------
