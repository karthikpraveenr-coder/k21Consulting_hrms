...
const EditRaiseTicket = () => {
    ...
    useEffect(() => {
        if (attachment) {
            document.querySelector("#img").setAttribute("src", `https://k21consulting.com/api/storage/app/${attachment}`);
        }
    }, [attachment]);
    ...
    useEffect(() => {
        const fetchAssetId = async () => {
            try {
                const response = await axios.get(`https://k21consulting.com/api/public/api/editview_raiselist/${id}`, {
                    headers: {
    ...
    const handleSave = (e) => {
        ...
        axios.put('https://k21consulting.com/api/public/api/update_raiseticket', formData, {
            headers: {
                'Content-Type': 'application/json',
    ...
    useEffect(() => {
        if (assignDepartment) {
            const apiUrl = `https://k21consulting.com/api/public/api/employee_dropdown_list/${assignDepartment}`;
            const fetchEmployees = async () => {
                try {
                    const response = await axios.get(apiUrl, {
                        headers: {
    ...
    useEffect(() => {
        const fetchDepartments = async () => {
            try {
                const response = await axios.get('https://k21consulting.com/api/public/api/issue_type_list', {
                    headers: {
                        Authorization: `Bearer ${usertoken}`
                    }
                });
