...
export default function EditPurchaseInvoice() {
    ...
    useEffect(() => {
        ...
        axios.get(`https://k21consulting.com/api/public/api/editview_purchaseinvoice/${id}`, {
            headers: {
    ...
    const handleSave = async (e) => {
        ...
        try {
            const response = await fetch('https://k21consulting.com/api/public/api/updatepurchaseinvoice', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${usertoken}`
    ...
    useEffect(() => {
        ...
        fetch('https://k21consulting.com/api/public/api/sales_company_list', {
            headers: {
                'Authorization': `Bearer ${usertoken}`
            }
        })
    ...
    //     fetch('https://k21consulting.com/api/public/api/autogeneratesaleinvoiceid', {
    //         headers: {
    //             'Authorization': `Bearer ${usertoken}`
    //         }
    //     })
    ...
    useEffect(() => {
        fetch('https://k21consulting.com/api/public/api/sales_item_list', {
    ...
    useEffect(() => {
        const fetchHsnSacOptions = async () => {
            ...
            for (const [index, item] of items.entries()) {
                if (item.descriptionalGoods) {
                    try {
                        const response = await fetch(`https://k21consulting.com/api/public/api/sales_hsn_sac/${item.descriptionalGoods}`, {
                            headers: { 'Authorization': `Bearer ${usertoken}` }
                        });
                        const data = await response.json();
                        if (data.status === 'success') {
    ...
    const handleChange = async (index, field, value) => {
        ...
        if (field === 'descriptionalGoods') {
            ...
            if (selectedOption) {
                try {
                    const response = await fetch(`https://k21consulting.com/api/public/api/sales_hsn_sac/${value}`, {
                        headers: { 'Authorization': `Bearer ${usertoken}` }
                    });
                    const data = await response.json();
                    if (data.status === 'success') {
