...
export default function EditAppointmentLetter() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('https://k21consulting.com/api/public/api/update_appointment_letter', formData, {
                headers: {
    ...
    useEffect(() => {
        axios.get(`https://k21consulting.com/api/public/api/edit_appointment_list/${id}`, {
            headers: {
                'Authorization': `Bearer ${usertoken}`
            }
        })
    ...
    useEffect(() => {
        if (headerAttachment && headerAttachment instanceof File) {
            ...
            setImagePreviewUrl(`https://k21consulting.com/api/storage/app/${headerAttachment}`);
        }
    ...
    useEffect(() => {
        if (footerAttachment && footerAttachment instanceof File) {
            ...
            setFooterImagePreviewUrl(`https://k21consulting.com/api/storage/app/${footerAttachment}`);
        }
        // Cleanup URL when component unmounts or file changes
        return () => {
